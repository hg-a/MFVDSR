			...
			for step in range(len(train_list)//BATCH_SIZE):
				...
				if step <= 0 and epoch == 0:
					...
					for i in range(0,19):
						...
						obser_tensor.sort()
						makecountlist(obser_tensor, -4525, 4526, i, step, 1, "before_ReLU")
						obser_tensor = aftercheck[i].reshape(6885376)
						obser_tensor.sort()
						makecountlist(obser_tensor, -4525, 4526, i, step, 1, "after_ReLU")
							
					obser_tensor = inp.reshape(107584)
					obser_tensor.sort()
					makecountlist(obser_tensor, -4525, 4526, 0, step, 1, "input")
					obser_tensor = output.reshape(107584)
					obser_tensor.sort()
					makecountlist(obser_tensor, -4525, 4526, 19, step, 1, "final_output")
					obser_evetensor = evetensor.reshape(107584)
					obser_evetensor.sort()
					makecountlist(obser_evetensor, -4525, 4526, 19, step, 1, "just_before_apply_skipconnection_before_output")

					obser_tensor = checkweights[0].reshape(576)
					obser_tensor.sort()
					weight_dist_countlist(obser_tensor, -1505, 1506, 0, step, 1)
					for i in range(1,19):
						obser_tensor = checkweights[i*2].reshape(36864)
						obser_tensor.sort()
						weight_dist_countlist(obser_tensor, -305, 306, i, step, 1)
					obser_tensor = checkweights[38].reshape(576)
					obser_tensor.sort()
					weight_dist_countlist(obser_tensor, -305, 306, 19, step, 1)

				
				if step <= 2 and epoch == 59:
					for i in range(0,19):
						obser_tensor = beforecheck[i].reshape(6885376)
						obser_tensor.sort()
						makecountlist(obser_tensor, -4525, 4526, i, step, 16, "before_ReLU")
						obser_tensor = aftercheck[i].reshape(6885376)
						obser_tensor.sort()
						makecountlist(obser_tensor, -4525, 4526, i, step, 16, "after_ReLU")
					...
					obser_tensor = output.reshape(107584)
					obser_tensor.sort()
					makecountlist(obser_tensor, -100, 2000, 19, step, 16, "final_output")
					obser_evetensor = evetensor.reshape(107584)
					obser_evetensor.sort()
					makecountlist(obser_evetensor, -100, 2000, 19, step, 16, "just_before_apply_skipconnection_before_output")


					obser_tensor = checkweights[0].reshape(576)
					obser_tensor.sort()
					weight_dist_countlist(obser_tensor, -1505, 1506, 0, step, 16)
					for i in range(1,19):
						obser_tensor = checkweights[i*2].reshape(36864)
						obser_tensor.sort()
						weight_dist_countlist(obser_tensor, -305, 306, i, step, 16)
					obser_tensor = checkweights[38].reshape(576)
					obser_tensor.sort()
					weight_dist_countlist(obser_tensor, -305, 306, 19, step, 16)

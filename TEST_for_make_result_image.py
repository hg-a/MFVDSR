...
import glob, os, scipy.io, time, argparse, scipy, scipy.misc
...
def test_VDSR_with_sess(ckpt_path, data_path, sess, weights, epo, li):
	...
	for folder_path in folder_list:
		...
		for i in range(len(img_list)):
			...
			if((i+1) % 3 == 1):
				scipy.misc.imsave("./gen test image/x2/%s/vdsr_no.%s_%0.3f.bmp" % (li, fn, psnr_vdsr), img_vdsr_y)
			if((i+1) % 3 == 2):
				scipy.misc.imsave("./gen test image/x3/%s/vdsr_no.%s_%0.3f.bmp" % (li, fn, psnr_vdsr), img_vdsr_y)
			if((i+1) % 3 == 0):
				scipy.misc.imsave("./gen test image/x4/%s/vdsr_no.%s_%0.3f.bmp" % (li, fn, psnr_vdsr), img_vdsr_y)
...
if __name__ == '__main__':
	if not os.path.exists('gen test image'):
		os.mkdir('gen test image')
	for i in range(2,5):
		if not os.path.exists('gen test image/x%d'%(i)):
			os.mkdir('gen test image/x%d'%(i))
		for p in all_dataset_list:
			if not os.path.exists('gen test image/x%d/%s'%(i,p)):
				os.mkdir('gen test image/x%d/%s'%(i,p))
